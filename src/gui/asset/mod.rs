...
impl AssetWidget {
    pub fn new() -> Self {
        Self {
            file_dialog: Self::build_file_dialog(),
            file_path: None,
            list_name: "List: none".to_string(),
            reversed: false,
            num_rows: 10,
        }
    }

    fn build_file_dialog() -> FileDialog {
        let path = PathBuf::from(&ASSET_DIR);

        FileDialog::new().initial_directory(path)
    }

    pub fn update(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
        ui.label(format!("Picked file: {:?}", self.file_path));
        ui.heading("Asset list widget");
        ui.horizontal(|ui| {
            if ui.button(&self.list_name).clicked() {
        ...
        table
            ...
            .body(|body| {
                body.rows(text_height, self.num_rows, |mut row| {
                    ...
                    row.col(|ui| {
                        ui.checkbox(&mut self.reversed, "Click me");
                        // ui.label("some text");
                    });
                });
            });
