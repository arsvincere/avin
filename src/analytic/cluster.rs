...
fn create_clusters_df(iid: &IID, tf: &TimeFrame) {
    ...
    while day < end {
        let tic_file = match DataFileTic::load(iid, &md, &day) {
            Ok(data) => data,
            Err(_) => {
...
fn analyse_quantum_feat(
    ...
    let name = analyse_name(tf, &Aspect::Quantum, Some(feat), Some(&analyse));
...
fn get_cdf_df(chart: &Footprint, feat: &Feat) -> Result<DataFrame, String> {
    ...
    let analyse = Analyse::CDF;
    let name =
        analyse_name(tf, &Aspect::Quantum, Some(&feat), Some(&analyse));
