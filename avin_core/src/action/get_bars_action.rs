...
///
/// Содержит канал для передачи ответа, идентификатор инструмента, таймфрейм,
/// начало и конец периода, закрытый диапазон DateTime Utc [from, till].
#[derive(Debug)]
...
impl GetBarsAction {
    ...
    ///
    /// # ru
    /// Создает новое действие с запросом баров у брокера.
    pub fn new(
        iid: Iid,
        tf: TimeFrame,
        from: DateTime<Utc>,
